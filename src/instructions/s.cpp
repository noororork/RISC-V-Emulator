...
S::S()
    ...
    sInstructions = {
        {{0x0, [this](int32_t imm, uint8_t rs1, uint8_t rs2){execute_sb(imm, rs1, rs2); }}, 
        {0x1, [this](int32_t imm, uint8_t rs1, uint8_t rs2){execute_sh(imm, rs1, rs2); }}, 
        {0x2, [this](int32_t imm, uint8_t rs1, uint8_t rs2){execute_sw(imm, rs1, rs2); }}
        }
    };
}
...
void S::execute_sb(int32_t imm, uint8_t rs1, uint8_t rs2){
    uint32_t address = registers[rs1] + imm;
    memory.writeByte(address, rs2);
}

void S::execute_sh(int32_t imm, uint8_t rs1, uint8_t rs2){
    uint32_t address = registers[rs1] + imm;
    memory.writeHalf(address, rs2);
}

void S::execute_sw(int32_t imm, uint8_t rs1, uint8_t rs2){
    uint32_t address = registers[rs1] + imm;
